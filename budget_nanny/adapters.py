...
def convert_bank_transactions_to_ynab_transactions(
    ...
    for bank_transaction in bank_transactions[:2]:
...
def convert_bank_transaction_to_ynab_transaction(
    ...
    matched_payee = payee_data_provider.get_fuzzy_match_on_ynab_payees_with_bank_payee(bank_transaction.payee)

    ynab_transaction = YnabTransaction(
        get_ynab_account_id_for_bank_account(bank_transaction.account),
        bank_transaction.date.date().isoformat(),
        int((bank_transaction.outflow * -1) * 1000)
        if bank_transaction.outflow
        else int(bank_transaction.inflow * 1000),
        matched_payee.id,
        matched_payee.name,
        'Imported by Budget Nanny',
    )

    ynab_transaction.import_id = _get_import_id_for_transaction(ynab_transaction)

    return ynab_transaction


def _get_import_id_for_transaction(transaction: YnabTransaction):
    """
